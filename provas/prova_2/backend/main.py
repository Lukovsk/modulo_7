...
def create_user(data: dict = Body()):
    cur.execute(
        f"INSERT INTO minhas_notas (titulo, descricao) VALUES ('{data['titulo']}', '{data['descricao']}');"
    )
    con.commit()
...
def update_user(data: dict = Body()):
    cur.execute(
        f"UPDATE minhas_notas SET titulo = '{data['titulo']}', descricao = '{data['descricao']}' WHERE id = {data['id']};"
    )
    con.commit()
